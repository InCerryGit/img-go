...
Markdown格式支持使用Base64方式内嵌图片（大多数Markdown编辑器都支持），这有着不需要付费图床，并且容易分发等优点。缺点就是如果图片太大会造成Markdown文件非常大，可能会导致Markdown编辑器崩溃，不过总得来说支持它有好处。使用方式非常简单，只需要加上将默认存储配置为`Embed`或者在命令行使用时`-s Embed`即可。
